...
def visualize(
    ...
    for detection in detection_result.detections:
        if detection.categories[0].score > 0.75:
            ...
            print(object_pos_robot)
            # print("x", object_x, "y", object_y, "z", object_z)
            if(abs(object_pos_robot[1] < 0.07) and bbox.height > bbox.width*2):
                find_angle((object_pos_robot[0]*1000,object_pos_robot[2]*1000),is_grab)
            elif (abs(object_pos_robot[1] < 0.07) and bbox.height < bbox.width*2):
                find_angle((object_pos_robot[0]*1000,object_pos_robot[2]*1000 - 50),is_grab)
