...
LocalizationOut::LocalizationOut()
{
    main_optical_flow_pose_sub = nh.subscribe("/aruco_map/pose", 1, &LocalizationOut::handle_main_optical_flow_pose, this);
    local_position_sub = nh.subscribe("mavros/local_position/pose", 1, &LocalizationOut::handle_local_position, this);
    auto sub_global_position_local = nh.subscribe("/mavros/global_position/local",1,&LocalizationOut::handle_global_position_local, this);
    range_finder_sub = nh.subscribe("/rangefinder/range", 1, &LocalizationOut::handle_rangefinder, this);
    // Publish
...
void LocalizationOut::handle_main_optical_flow_pose(const geometry_msgs::PoseWithCovarianceStamped &msg)
{
...
void LocalizationOut::handle_local_position(const geometry_msgs::PoseStamped &msg)
{
...
void LocalizationOut::handle_global_position_local(const nav_msgs::Odometry &odom)
{
...
void LocalizationOut::handle_rangefinder(const sensor_msgs::RangeConstPtr &range)
{
    _rangefinder = *range;
}
...
LocalizationIn::LocalizationIn()
{
    main_optical_flow_pose_sub = nh.subscribe("/aruco_map/pose",1,&LocalizationIn::handle_main_optical_flow_pose, this);
    local_position_sub = nh.subscribe("mavros/local_position/pose",1,&LocalizationIn::handle_local_position, this);
    range_finder_sub = nh.subscribe("/rangefinder/range", 1, &LocalizationIn::handle_rangefinder, this);
    // Publish
    uavpose_pub = nh.advertise<geometry_msgs::PoseStamped>("uavlab411/uavpose", 1);
    path_pub = nh.advertise<nav_msgs::Path>("/pathuav", 10);
    pub_baro = nh.advertise<sensor_msgs::Range>("/rangefinder/range", 1);
...
//handle subscribe topic funtion
void LocalizationIn::handle_main_optical_flow_pose(const geometry_msgs::PoseWithCovarianceStamped& msg)
{
    //convert from main_camera_optical frame_id to aruco_map frame_id
    //main_camera_optical trung goc toa do voi map nhung goc xoay cung chieu voi yaw cua uav
    geometry_msgs::Pose2D pose2d;
...
void LocalizationIn::handle_local_position(const geometry_msgs::PoseStamped& msg)
{
    z_barometer = msg.pose.position.z;
    sensor_msgs::Range range;
...
void LocalizationIn::handle_rangefinder(const sensor_msgs::RangeConstPtr &range)
{
    _rangefinder = *range;
}
